...
public class PacPlayer implements Player {
    ...
    private static final int DOT_REMAINED = 15;
...
//                score += 40;
    ...
    public static Point findNearestDot(Maze maze, Pac pac) {
        ...
        double shortestDistToPac = Math.sqrt(width ^ 2 + height ^ 2);
    ...
    public MoveDir findBestDirToNearestDot(Maze maze, Pac pac, Ghost[] ghosts) {
        ...
        List<List<Point>> allPathToNearestDotTile = MazeGraph.getPaths(pacTile, nearestDotTile, NODE_LIMIT);
        List<Point> bestPath = allPathToNearestDotTile.get(0);

        for (List<Point> path : allPathToNearestDotTile) {
            int score = 0;
            score = getPathScore(maze, ghosts, score, path);
            if (score > maxScore) {
                maxScore = score;
                bestPath = path;
            }
        ...
        return getDirFromPoint(pac.getTile(), bestPath.get(0));
    }
}